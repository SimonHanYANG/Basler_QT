...
/*
	QMutexLocker是Qt中的一个类，用于在多线程应用程序中控制共享资源的访问。
	它提供了一种方便的方式来锁定一个QMutex对象，并在作用域结束时自动释放锁，
	从而避免了手动管理锁的复杂性和潜在的竞争条件。在使用QMutexLocker时，
	可以确保只有一个线程可以访问被锁定的资源，从而避免了数据竞争和其他多线程问题
*/
